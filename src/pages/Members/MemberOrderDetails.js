...
function MemberOrderDetails(props) {
  ...
  useEffect(() => {
    let getMemberOrderDetails = async () => {
      let response = await axios.get(`${API_URL}/member/${orderId}`);
    ...
    <>
      <div className="container mt-5 containerDetailsMember">
        ...
        <div className="row d-flex justify-content-center">
          ...
          <h2 className="text-center gray">合計：NT$</h2>
                    ...
                    {data.map((item) => {
                      return (
                        <tr>
                          ...
                          <td>
                            <p className="m-0">NT${item.product_price}</p>
                            <p className="text-decoration-line-through remarkMember">
                              NT$11,900
                            </p>
                          </td>
                          <td>{item.quantity}</td>
                          <td>NT${item.product_price * item.quantity}</td>
                        </tr>
                      );
                    })}
                    {/* <tr>
